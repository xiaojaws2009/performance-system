...
define('WORKER_PATH', realpath(APP_PATH . '/vendor/workerman/workerman'));
...
class Worker
{
    ...
    protected static function parseCommand()
    {
        ...
        switch ($command) {
            ...
            case 'status':
                ...
                exit(0);
            case 'kill':
                exec("ps aux | grep ".self::$_startFile." | grep -v grep | awk '{print $2}' |xargs kill -2");
                usleep(100000);
                exec("ps aux | grep ".self::$_startFile." | grep -v grep | awk '{print $2}' |xargs kill -9");
                exit(0);
            case 'connections':
    ...
    protected static function getEventLoopName()
    {
        ...
        if (!class_exists(WORKER_PATH . '\Swoole\Event')) {
        ...
        if ($loop_name) {
            if (interface_exists(WORKER_PATH . '\Events\React\EventLoop\LoopInterface')) {
                switch ($loop_name) {
                    case 'libevent':
                        static::$eventLoopClass = '\React\ExtLibEventLoop';
                        break;
                    case 'event':
                        static::$eventLoopClass = '\React\ExtEventLoop';
                        break;
                    default :
                        static::$eventLoopClass = '\React\StreamSelectLoop';
                        break;
                }
            } else {
            ...
            static::$eventLoopClass = interface_exists(WORKER_PATH . '\React\EventLoop\LoopInterface')? '\Workerman\Events\React\StreamSelectLoop':'\Workerman\Events\Select';
        }
        return static::$eventLoopClass;
    }
